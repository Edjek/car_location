    ...
    <div class="w-75 mx-auto mt-5 p-4 border rounded">
        <form action="<?= SITE_NAME; ?>/dashboard/users/modifier-user" method="post">
            ...
            <div class="mb-3">
                ...
                <input type="radio" id="email" class="form-check-input" value="true" name="user_statut" <?php echo ($user['statut'] == 1) ? 'checked' : ''; ?>>
                <label for="user" class="form-check-label">user</label>
                <input type="radio" id="statut" class="form-check-input" value="false" name="user_statut" <?php echo ($user['statut'] == 0) ? 'checked' : ''; ?>>
            </div>
